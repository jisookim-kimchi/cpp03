...
/*   Updated: 2026/03/05 08:29:34 by jisokim2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ScavTrap::attack(const std::string& target)
{
    if (_hitPoints == 0)
    {
        std::cout << "ScavTrap " << _name << " is dead can't attack" << std::endl;
        return;
    }
    if (_energyPoints == 0)
    {
        std::cout << "ScavTrap " << _name << " has no energyPoints can't attack" << std::endl;
        return;
    }
    std::cout << "ScavTrap " << _name << " attack " << target << " casuing " << _attackDamage << " points of damage!"<< std::endl;
    _energyPoints--;
}